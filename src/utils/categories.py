...
CATEGORIES = {
    "Документы": CategoryInfo(
        ...
        text=(
            ...
            "Он находится в 11-ом корпусе в самом конце коридора."
        )
    ),
    ...
    "Служба заботы": CategoryInfo(
        ...
        text=(
            ...
            "Он находится в 15-ом корпусе во 2-ом крыле 3-его этажа, рядом с аудиториями 3.10 и 3.11."
        )
    ),
    "Другое": CategoryInfo(
...
SUBCATEGORIES = {
    "Обратная связь": CategoryInfo(
        ...
        text=(
            ...
            "👾Напоминаем, пока ваше обращение не рассмотренно — вы не можете создать новое. "
            "Бот не предназначен для ведения диалога и не получает ваши ответы👾"
        )
    ),
    ...
    "Запрос на печать": CategoryInfo(
        image="assets/images/other.jpg",
        text=(
            "Если тебе необходимо распечатать материал — подробно распиши, как именно это нужно сделать. "
            "Мы рекомендуем отправлять материалы заранее вместе с описанием:\n\n"
            "1. Сколько экземпляров необходимо? Какие страницы нужны?\n"
            "2. Односторонняя или двухсторонняя печать?\n"
            "3. К какому времени необходимо распечатать данные материалы? В каком корпусе будет удобно их забрать?\n"
            "4. Дополнительная информация.\n\n"
            "Файл необходимо прикрепить в одном сообщении с запросом. Если ваш запрос не соответствует форме — он не будет принят.\n\n"
            "⚫️ Обратите внимание, печать осуществляется только в чёрно-белом формате. ⚪️\n\n"
            "👾 Напоминаем: пока ваше обращение не рассмотрено — вы не можете создать новое. "
            "Бот не предназначен для ведения диалога и не получает ваши ответы. 👾"
        )
    )
}
...
ACKNOWLEDGMENT_CAPTION = (
    "Спасибо! Твое сообщение отправлено в службу поддержки.\n\n"
    "👾Напоминаем, пока ваше обращение не рассмотренно — вы не можете создать новое. Бот не предназначен для ведения диалога и не получает ваши ответы👾"
)
